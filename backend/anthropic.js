...
- \`parallax-bg\` on a \`<section>\` that has a **visual background**. Two supported patterns; the runtime auto-detects which:
  - **CSS background-image pattern**: the section carries \`background-image: url(...)\` directly (inline or via a CSS rule targeting that section). The runtime lifts it into a pseudo-element it can transform.
  - **Direct \`<img>\` child pattern** (common hero layout): the section has an \`<img>\` as a direct child, positioned to fill it (\`position: absolute; inset: 0; width: 100%; height: 100%; object-fit: cover;\`). The runtime transforms the img directly and oversizes it via inline style so translation doesn't reveal gaps.
  - **Will NOT work** on sections whose only "background" is a solid color, a gradient, or an image sitting in the content flow. Do NOT put \`parallax-bg\` on an empty background-only wrapper div sitting behind sibling overlay/content layers — put it on the section that contains the content, and use a \`::before\`/overlay child for any darkening tint.
...
- **Parallax** — \`parallax-bg\` on a \`<section>\` with either a CSS \`background-image\` OR a direct \`<img>\` child positioned to fill it (\`position:absolute; inset:0; object-fit:cover\`) — both patterns work, the runtime auto-detects. Won't work on solid-color, gradient, or in-flow imagery. OR \`data-parallax="<speed>"\` on any element (0.6–0.85 typical) — good for text-only depth (e.g., side-by-side cards at different speeds). Multiple per page OK when they reinforce. Runtime auto-disables on touch/narrow viewports — the layout must still work as a static stack.
